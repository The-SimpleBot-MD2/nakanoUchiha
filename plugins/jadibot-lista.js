...
async function handler(m, { conn: _envio, usedPrefix }) {
  // Obtiene una lista única de conexiones activas con usuarios que no están cerrados.
  const users = [...new Set([...global.conns.filter((conn) => conn.user && conn.ws.socket && conn.ws.socket.readyState !== ws.CLOSED).map((conn) => conn)])];

  // Función para convertir milisegundos a un formato más legible de días, horas, minutos y segundos.
  ...
  // Crea el mensaje para cada sub-bot con su información y el tiempo que lleva activo.
  ...
  // Si no hay sub-bots disponibles, envía un mensaje indicando eso.
  const replyMessage = message.length === 0 ? '*NO HAY SUB BOTS DISPONIBLE. VERIFIQUE MÁS TARDE.*' : message;
  
  // Cuenta el total de usuarios sub-bots activos.
  const totalUsers = users.length;
  
  // Mensaje de respuesta completo con la lista de sub-bots y la cantidad total.
  ...
  // Envía el mensaje de respuesta al chat.
